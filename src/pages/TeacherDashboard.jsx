...
// import { useState, useEffect } from 'react';
// import { PHASE_MAP, ageFromBirth } from '@/lib/phases';
// import { Button } from '@/components/ui/button';
// import { GraduationCap, Users, RefreshCw, LogOut, Trash2 } from 'lucide-react';
...
//       localStorage.setItem('saber_brilhante_alunos', JSON.stringify(list));
//       setStudents(list);
//       setNotice(`${name} foi removido com sucesso.`);
//       setTimeout(() => setNotice(null), 3000);
//     } catch (e) {
//       console.error(e);
//     }
...
//           <div>
//             <h1 className="text-2xl font-extrabold text-violet-800">Área da Professora</h1>
//             <p className="text-sm text-muted-foreground">Acompanhe a fase de cada aluno local.</p>
//           </div>
//         </div>
//         <Button variant="outline" onClick={lock}>
//           <LogOut className="w-4 h-4 mr-1" /> Sair da área
...
//           <Users className="w-10 h-10 text-violet-300 mx-auto" />
//           <p className="mt-2 font-semibold text-slate-700">Nenhum aluno cadastrado ainda.</p>
//           <p className="text-sm text-muted-foreground">Os alunos aparecem aqui assim que realizarem o cadastro na área deles.</p>
//         </div>
//       ) : (
//         <div className="grid md:grid-cols-2 gap-4">
//           {students.map((s) => {
...
//             const ph = PHASE_MAP[s.phase] || PHASE_MAP.garatuja;
//             return (
//               <div key={s.id} className="bg-white rounded-3xl border border-violet-100 shadow p-5 flex items-center justify-between">
//                 <div className="flex items-center gap-3">
//                   <div className="text-3xl">{ph.emoji}</div>
//                   <div>
//                     <h3 className="font-extrabold text-slate-800">{s.name}</h3>
//                     <p className="text-xs text-muted-foreground">{age} anos • Fase: <b className="text-violet-700">{ph.label}</b></p>
//                   </div>
//                 </div>
//                 <button onClick={() => confirmDelete(s.id, s.name)} title="Excluir aluno" className="w-8 h-8 rounded-full bg-red-50 text-red-500 hover:bg-red-100 flex items-center justify-center transition">
//                   <Trash2 className="w-4 h-4" />
//                 </button>
//               </div>
//             );
...
export default function TeacherDashboard() {
  ...
  const updateStudent = async (id, data) => {
    try {
      const currentData = localStorage.getItem('saber_brilhante_alunos');
      let list = currentData ? JSON.parse(currentData) : [];
  ...
  const clearReforco = async (id) => {
    await updateStudent(id, { reforco_message: '', reforco_game: '' });
    setNotice('Reforço limpo com sucesso.');
          ...
          {students.map((s) => {
            ...
            return (
              <div key={s.id} className="bg-white rounded-3xl border border-violet-100 shadow p-5 space-y-3">
                ...
                {reforcoId === s.id && (
                  <div className="rounded-2xl bg-amber-50 border border-amber-100 p-3 space-y-2 mt-2">
                    <p className="text-xs font-bold text-amber-800">Enviar recadinho de reforço para {s.name}:</p>
                    <textarea value={reforcoMsg} onChange={(e) => setReforcoMsg(e.target.value)} placeholder="Ex: Vamos praticar um pouquinho mais as sílabas, você está indo muito bem!" rows={2} className="w-full px-3 py-2 rounded-lg border-2 border-amber-100 focus:border-amber-400 outline-none text-sm resize-none" />
                    <Select value={reforcoGame} onValueChange={setReforcoGame}>
                      <SelectTrigger className="h-8 text-xs"><SelectValue placeholder="Sugerir um jogo (opcional)" /></SelectTrigger>
                ...
                <Button size="sm" className="w-full bg-violet-600 hover:bg-violet-700 mt-2" onClick={() => requestTest(s)}>
                  <RefreshCw className="w-4 h-4 mr-1" /> Iniciar novo teste
                </Button>
              </div>
            );
